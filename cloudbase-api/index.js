...
exports.main = async function main(event) {
  ...
  try {
    ...
    if (method === "POST" && path.endsWith("/api/user/login")) return await handleUserLogin(event, headers);
...
// Password-based user auth overrides. These later declarations replace the
// earlier no-password MVP handlers without touching the legacy code above.
async function handleUserRegister(event, headers) {
  const { userId, password } = parseBody(event);
  const normalized = normalizeUserId(userId);
  const invalid = validateUserId(normalized);
  if (invalid) return json(400, invalid, headers);
  const invalidPassword = validatePassword(password);
  if (invalidPassword) return json(400, invalidPassword, headers);

  const existing = await getUser(normalized);
  if (existing) {
    return json(409, {
      error: "USER_ID_TAKEN",
      message: "该用户ID已存在，请登录。"
    }, headers);
  }
  ...
  return json(200, {
    token: signToken({ role: "user", userId: normalized, exp: Date.now() + 30 * 24 * 60 * 60 * 1000 }, userTokenSecret()),
    user: publicUser(user)
  }, headers);
}

async function handleUserLogin(event, headers) {
  const { userId, password } = parseBody(event);
  const normalized = normalizeUserId(userId);
  const invalid = validateUserId(normalized);
  if (invalid) return json(400, invalid, headers);
...
async function handleMe(event, headers) {
  ...
  const user = await getUser(auth.payload.userId);
  if (!user) {
    return json(404, {
      error: "USER_NOT_FOUND",
      message: "用户不存在，请先注册。"
  ...
  await touchUser(auth.payload.userId);
  return json(200, publicUser(user), headers);
}
...
async function handleMatch(event, headers) {
  ...
  if (!apiKey) {
    return json(503, {
      error: "MISSING_DEEPSEEK_API_KEY",
      message: "后端没有配置 DEEPSEEK_API_KEY。"
    }, headers);
  }
  ...
  try {
    ...
    return json(502, { error: "DEEPSEEK_NON_JSON", message: "DeepSeek 返回了非 JSON 响应。", detail: raw.slice(0, 300) }, headers);
  }
  ...
  try {
    ...
    return json(502, { error: "DEEPSEEK_REPORT_INVALID_JSON", message: "DeepSeek 的分析结果不是合法 JSON。", detail: content.slice(0, 300) }, headers);
  }
...
function validatePassword(password) {
  if (String(password || "").length < 8) {
    return { error: "INVALID_PASSWORD", message: "密码至少需要 8 位。" };
  }
  return null;
}
...
function userTokenSecret() {
  return process.env.USER_TOKEN_SECRET || process.env.ADMIN_TOKEN_SECRET || process.env.ADMIN_PASSWORD || "change-me";
}

function requireUser(event) {
  const auth = getHeader(event, "authorization");
  const token = auth && auth.startsWith("Bearer ") ? auth.slice(7) : "";
  const payload = verifyToken(token, userTokenSecret());
  if (!payload || payload.role !== "user" || !payload.userId) {
    return {
      status: 401,
      error: { error: "USER_UNAUTHORIZED", message: "请先登录。" }
    };
  }
  return { payload };
}

function signToken(payload, secret = process.env.ADMIN_TOKEN_SECRET || process.env.ADMIN_PASSWORD || "change-me") {
  const body = Buffer.from(JSON.stringify(payload)).toString("base64url");
  const sig = crypto.createHmac("sha256", secret).update(body).digest("base64url");
  return `${body}.${sig}`;
}

function verifyToken(token, secret = process.env.ADMIN_TOKEN_SECRET || process.env.ADMIN_PASSWORD || "change-me") {
  try {
    const [body, sig] = String(token || "").split(".");
    if (!body || !sig) return null;
    const expected = crypto.createHmac("sha256", secret).update(body).digest("base64url");
